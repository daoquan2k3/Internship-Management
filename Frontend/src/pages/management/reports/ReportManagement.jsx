...
const ReportManagement = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    const fetchReportsEffect = async () => {
      try {
        setLoading(true);
        const res = await reportApi.getAllReports();
        if (isMounted) setReports(res?.content || []);
      } catch (err) {
        console.error("Lỗi khi tải báo cáo:", err);
      } finally {
                ...
                >
                  ...
                  >
                    ...
                    <Stack direction="row" alignItems="flex-start" sx={{ position: "relative", zIndex: 1, mb: 3, pr: 8 }}>
                      <Box sx={{ p: 1.5, borderRadius: 3, bgcolor: "#e3f2fd", color: "primary.main", mr: 2 }}>
                        <AssignmentTurnedInIcon />
                      </Box>
                      <Box>
                        <Typography variant="h6" sx={{ fontWeight: 800, color: "primary.light", lineHeight: 1.2, mb: 0.5, wordBreak: "break-word", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                          {report.title}
                        </Typography>
                    ...
                    <Stack spacing={1.5} sx={{ mb: 3, position: "relative", zIndex: 1, bgcolor: "background.paper", p: 2, borderRadius: 3, flexGrow: 1 }}>
                      <Stack direction="row" alignItems="center" gap={1.5}>
                        <BadgeIcon sx={{ color: "#757575", fontSize: 18 }} />
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>Mã SV: <span style={{ fontWeight: 700, color: "primary.main" }}>{report.studentCode}</span></Typography>
                      </Stack>
                      <Stack direction="row" alignItems="center" gap={1.5}>
                        <PersonIcon sx={{ color: "#757575", fontSize: 18 }} />
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>Họ tên: <span style={{ fontWeight: 500, color: "text.primary" }}>{report.studentName}</span></Typography>
                      </Stack>
                      <Stack direction="row" alignItems="center" gap={1.5}>
                    ...
                    {report.aiSummary && (
                      <Box sx={{ mb: 3, p: 2, borderRadius: 2, bgcolor: 'rgba(156, 39, 176, 0.05)', border: '1px solid rgba(156, 39, 176, 0.2)', position: 'relative', zIndex: 1 }}>
                        ...
                        <Typography variant="body2" sx={{ fontWeight: 'bold', color: "text.primary" }}>Tóm tắt:</Typography>
                        <Typography variant="body2" sx={{ mb: 1, color: "text.primary", pl: 1, borderLeft: '2px solid #9c27b0' }}>{report.aiSummary}</Typography>
                    ...
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 1, position: "relative", zIndex: 1 }}>
                      ...
                      <Box sx={{ display: "flex", justifyContent: "space-between", gap: 1 }}>
                        ...
                        <Button
                          ...
                          sx={{ borderRadius: 2, fontWeight: 700, flex: 1, boxShadow: "0 4px 10px rgba(21, 101, 192, 0.2)", bgcolor: "primary.main", "&:hover": { bgcolor: "primary.dark" } }}
                        >
                          TẢI XUỐNG
                        </Button>
                      </Box>
